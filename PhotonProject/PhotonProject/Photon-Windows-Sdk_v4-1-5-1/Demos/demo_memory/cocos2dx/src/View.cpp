...
bool View::onTextFieldInsertText(cocos2d::CCTextFieldTTF* pSender, const ExitGames::Common::JString& text, int nLen)
{
	return false;
}
...
void View::roomListCallback(const ExitGames::Common::JString& name)
{
	mpLbl->joinRoom(name);
}

void View::savedRoomListCallback(const ExitGames::Common::JString& name)
{
	mpLbl->joinSavedRoom(name);
}

void View::onStateChange(int state, const ExitGames::Common::JString& stateStr, bool inLobby, bool inRoom)
{
...
void View::refreshStateInfo(const ExitGames::Common::JString& stateInfo)
{
	mpLabelState->setString((stateInfo?(mStateString+L" "+stateInfo):mStateString).UTF8Representation().cstr());
}

void View::refreshMessage(const ExitGames::Common::JString& msg)
{
	mpLabelMessage->setString(msg.UTF8Representation().cstr());
}

void View::updateRoomList(const ExitGames::Common::JString roomNames[], unsigned int size)
{
...
void View::updateSavedRoomList(const ExitGames::Common::JString roomNames[], unsigned int size)
{
	mpSavedRoomList->removeAllItems();
	for(unsigned int i=0; i<size; ++i)
		mpSavedRoomList->addItem(roomNames[i]);
}
