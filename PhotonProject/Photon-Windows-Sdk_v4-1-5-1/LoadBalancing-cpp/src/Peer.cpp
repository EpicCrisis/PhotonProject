...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		bool Peer::opJoinRoom(const JString& gameID, const RoomOptions& options, const Hashtable& customLocalPlayerProperties, bool createIfNotExists, int playerNumber, int cacheSliceIndex, const JVector<JString>& expectedUsers)
		{
			return gameID.length() ? opCustom(OperationRequest(OperationCode::JOIN_ROOM, opJoinRoomImplementation(gameID, options, customLocalPlayerProperties, createIfNotExists, playerNumber, cacheSliceIndex, expectedUsers)), true) : false;
		}
		...
		bool Peer::opAuthenticateOnce(const JString& appID, const JString& appVersion, nByte connectionProtocol, nByte encryptionMode, const AuthenticationValues& authenticationValues, bool lobbyStats, const JString& regionCode)
		{
			...
			return opCustom(OperationRequest(OperationCode::AUTH_ONCE, op), true, 0);
		}
		...
		OperationRequestParameters Peer::opJoinRoomImplementation(const JString& gameID, const RoomOptions& options, const Hashtable& customLocalPlayerProperties, bool createIfNotExists, int playerNumber, int cacheSliceIndex, const JVector<JString>& expectedUsers)
		{
