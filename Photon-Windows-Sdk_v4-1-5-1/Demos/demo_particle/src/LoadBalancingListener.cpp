...
bool LoadBalancingListener::setLocalPlayerPos(int x, int y)
{
	if(x >= 0 && x < mGridSize && y >= 0 && y < mGridSize) 
	{
		...
		if(mSendGroup)
			mpLbc->opRaiseEvent(false, data, 2, RaiseEventOptions().setInterestGroup(mSendGroup));
		else
		{
			if(mUseGroups)
				mpLbc->opRaiseEvent(false, data, 2, RaiseEventOptions().setInterestGroup(getGroupByPos()));
			else
...
void LoadBalancingListener::raiseColorEvent(void)
{
	...
	if(mSendGroup)
		mpLbc->opRaiseEvent(true, data, 1, RaiseEventOptions().setEventCaching(ExitGames::Lite::EventCache::ADD_TO_ROOM_CACHE));
}
