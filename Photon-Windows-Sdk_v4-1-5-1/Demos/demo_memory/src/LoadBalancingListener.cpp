...
void LoadBalancingListener::connectReturn(int errorCode, const JString& errorString, const JString& cluster)
{
    ...
    if(errorCode == ErrorCode::OK)
		EGLOG(DebugLevel::INFO, L"connected to cluster " + cluster);
	else
...
void LoadBalancingListener::joinSavedRoom(const JString& gameId)
{
	...
	if(info)
	{
		...
		if(actorNrObj)
		{
			...
			EGLOG(DebugLevel::INFO, L"Joining Saved Room: %ls", gameId.cstr(), ", actorNr: %d", actorNr);
			mpLbc->opJoinRoom(gameId, false, actorNr);
			return;
		}
	}
