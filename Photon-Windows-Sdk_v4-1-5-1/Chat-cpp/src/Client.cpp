...
namespace ExitGames
{
	namespace Chat
	{
		...
		/** @copydoc PhotonPeer::getPeerCount */ 
		short Client::getPeerCount(void)
		{
			return Peer::getPeerCount();
		}

		/** @copydoc PhotonPeer::getQuickResendAttempts */ 
...
#	pragma warning(pop)
		...
		/** @fn template<typename Ftype> bool Client::opPublishMessage(const Common::JString& channelName, Ftype message)
		...
		/** @fn template<typename Ftype> bool Client::opPublishMessage(const Common::JString& channelName, Ftype pMessageArray, typename Common::Helpers::ArrayLengthType<Ftype>::type arrSize)
		    @overload
		    @param channelName channel name
		...
		/** @fn template<typename Ftype> bool Client::opPublishMessage(const Common::JString& channelName, Ftype pMessageArray, const short* pArrSizes);
			@overload
		    @param channelName channel name
		    @param pMessageArray message to send
		...
		bool Client::connect(AuthenticationValues authenticationValues, const JString& nameServerAddress)
		{
			mAuthenticationValues = authenticationValues;
...
#endif
			if(mPeer.connect(nameServerAddressWithoutProtocol + (nameServerAddressWithoutProtocol.indexOf(L':') == -1?JString(L":")+(port+NAMESERVER_PORT_GAP):JString()), mAppIDPeerInit))
			{
